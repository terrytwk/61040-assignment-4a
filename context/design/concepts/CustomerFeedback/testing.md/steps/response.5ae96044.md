---
timestamp: 'Sat Oct 18 2025 20:20:22 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251018_202022.37f51a60.md]]'
content_id: 5ae96044f7a25fa1da363cb54fb013bd81323d1828dd1fde1aa8c91ce5c8b5d6
---
...
export type User = ID; // Exporting these types for use in test files
export type Order = ID; // Exporting these types for use in test files
export type Feedback = ID; // The unique identifier for a feedback record itself
...
// Import User and Order types from the concept file
import CustomerFeedbackConcept, { User, Order } from "./CustomerFeedbackConcept.ts";
...
Deno.test("CustomerFeedbackConcept", async (t) => {
  ...
  const userA: User = "user:alice" as User;
  const userB: User = "user:bob" as User;
  const order1: Order = "order:123" as Order;
...
//    - Expect three entries, one for `order:123`, one for `order:456`, and one for `order:new4` (added in test), all with correct comments and timestamps.
