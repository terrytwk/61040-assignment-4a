...
content_id: a442233edba04ec8e0d4616d6f462fe8dc60cfcda11ed65e7dbda2d4818125f5
---
...
export default class MenuConcept {
  ...
  async _isSelectionSetValid({ item, selections }: {
    ...
    // Fetch all attached applicabilities for the given item
    ...
    // Consolidate all option and choice IDs involved in attached applicabilities and current selections
    const allOptionIdsInvolved = new Set<Option>();
    const allChoiceIdsInvolved = new Set<Choice>();

    for (const app of attachedApplicabilities) {
      allOptionIdsInvolved.add(app.option);
    }
    for (const selection of selections) {
      allOptionIdsInvolved.add(selection.option);
      allChoiceIdsInvolved.add(selection.choice);
    }
    ...
    const [
      ...
      this.options.find({ _id: { $in: Array.from(allOptionIdsInvolved) } }).toArray(),
      this.choices.find({ _id: { $in: Array.from(allChoiceIdsInvolved) } }).toArray(),
    ]);
    ...
    // Group selections by option to easily count choices per option and check for duplicates
    ...
    // 1. Each selected option must be attached to the item.
    // 2. Each selected choice must belong to its option, be active, and not be disallowed for the item/option.
    for (const selection of selections) {
      const { option, choice } = selection;

      // 1. Check if selected option is attached to item
      if (!attachedOptionIds.has(option)) {
        return [{ ok: false, reason: `Selected option ${option} is not attached to item ${item}.` }];
      }

      // 2. Check if choice belongs to its option, is active, and is not disallowed
      const choiceDoc = choiceMap.get(choice);
      if (!choiceDoc || choiceDoc.option !== option) {
        return [{ ok: false, reason: `Selected choice ${choice} does not belong to option ${option}.` }];
      }
      if (!choiceDoc.isActive) {
        return [{ ok: false, reason: `Selected choice ${choice} is not active.` }];
      }

      const applicability = applicabilityMap.get(option);
      if (applicability && applicability.disallowedChoices.includes(choice)) {
        return [{ ok: false, reason: `Selected choice ${choice} is disallowed for option ${option} on item ${item}.` }];
      }
    }

    // 3. All attached required options must be present in selections (if required)
    for (const app of attachedApplicabilities) {
      const optionDoc = optionMap.get(app.option);
      if (optionDoc?.required && !selectionsByOption.has(app.option)) {
        return [{ ok: false, reason: `Required option ${app.option} is missing for item ${item}.` }];
      }
    }

    // 4. Per-option maxChoices must be respected, and no duplicate choices within an option selection
    for (const [optionId, selectedChoices] of selectionsByOption.entries()) {
      const optionDoc = optionMap.get(optionId);
      if (optionDoc && selectedChoices.length > optionDoc.maxChoices) {
        return [{ ok: false, reason: `Option ${optionId} exceeds its maximum allowed choices (${optionDoc.maxChoices}).` }];
      }
      // Check for duplicate choices within the same option selection
      if (new Set(selectedChoices).size !== selectedChoices.length) {
        return [{ ok: false, reason: `Duplicate choices provided for option ${optionId}.` }];
      }
    }
