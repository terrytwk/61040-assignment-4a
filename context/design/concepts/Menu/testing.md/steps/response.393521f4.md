---
timestamp: 'Sat Oct 18 2025 19:57:26 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251018_195726.0e456588.md]]'
content_id: 393521f463bf0f28b5a66b77520b286908719c301ae5bf23254662345c3e4dcb
---
...
Deno.test("Menu Concept - Actions", async (t) => {
  ...
  await t.step("Principle: Operators attach temperature and milk to Latte; only temperature to Americano. The POS shows the right choices and blocks disallowed combos.", async () => {
    ...
    // Re-attach milk to Americano for demonstrating POS logic
    ...
    const latteOptions = (await menu._optionsForItem({ item: latteItem })) as { option: { id: ID, required: boolean, maxChoices: number } }[];
    ...
    const americanoOptions = (await menu._optionsForItem({ item: americanoItem })) as { option: { id: ID, required: boolean, maxChoices: number } }[];
    assertEquals(americanoOptions.length, 1); // Only Temperature (Milk was detached)
    assert(americanoOptions.some(o => o.option.id === milkOption && !o.option.required && o.option.maxChoices === 1));
    console.log(`Americano has Milk (optional, max 1).`); // Should be just milk if temp detached earlier

    // Recalculate options for Americano because of earlier detach and re-attach scenario
    const _optionsForItemAmericanoResult = await menu._optionsForItem({ item: americanoItem });
    if ("error" in _optionsForItemAmericanoResult) throw new Error(_optionsForItemAmericanoResult.error);
    const americanoActualOptions = _optionsForItemAmericanoResult;
    const americanOptionIds = americanoActualOptions.map(o => o.option.id);
    assert(americanOptionIds.includes(milkOption), "Americano should have Milk option (re-attached)");
    assertFalse(americanOptionIds.includes(temperatureOption), "Americano should NOT have Temperature option (detached)");


    console.log(`Query: _choicesFor (Latte, Milk) - checking disallowed combo`);
    const latteMilkChoices = (await menu._choicesFor({ item: latteItem, option: milkOption })) as { choice: { id: ID, name: string } }[];
    ...
    const validation1 = (await menu._isSelectionSetValid({ item: latteItem, selections: selections1 })) as { ok: boolean, reason?: string }[];
    ...
    const validation2 = (await menu._isSelectionSetValid({ item: latteItem, selections: selections2 })) as { ok: boolean, reason?: string }[];
    ...
    const validation3 = (await menu._isSelectionSetValid({ item: latteItem, selections: selections3 })) as { ok: boolean, reason?: string }[];
    ...
    const validation4 = (await menu._isSelectionSetValid({ item: latteItem, selections: selections4 })) as { ok: boolean, reason?: string }[];
